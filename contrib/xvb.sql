...
INSERT INTO VPBX_REPORTS (NAME,TYPE,CREATE_TIMESTAMP,QUERY,TTL) values('Call terminated',1,unix_timestamp(),"select 'PBX (die)' as '=Term', count(*) as 'count=' from VPBX_CDRS where DATA like '%TERM=PBX (die)%' union select 'PBX (unauthorized)' as '=Term', count(*) as 'count=' from VPBX_CDRS where DATA like '%TERM=PBX (unauthorized)%' union select 'PBX (c-limit)' as '=Term', count(*) as 'count=' from VPBX_CDRS where DATA like '%TERM=PBX (c-limit)%' union select 'PBX (u-c-limit)' as '=Term', count(*) as 'count=' from VPBX_CDRS where DATA like '%TERM=PBX (u-c-limit)%' union select 'PBX (locked)' as '=Term', count(*) as 'count=' from VPBX_CDRS where DATA like '%TERM=PBX (locked)%' union select 'PBX' as '=Term', count(*) as 'count=' from VPBX_CDRS where DATA like '%TERM=PBX' union select 'in - IVR' as '=Term', count(*) as 'count=' from VPBX_CDRS where DATA like '%TERM=IVR%' union select 'out - SRC' as '=Term', count(*) as 'count=' from VPBX_CDRS where DATA like '%TERM=SRC%' union select 'out - DST' as '=Term', count(*) as 'count=' from VPBX_CDRS where DATA like '%TERM=DST%' union select 'in - USER' as '=Term', count(*) as 'count=' from VPBX_CDRS where DATA like '%TERM=USER%'",300);
...
insert into VPBX_SIPPEERS_TEMPLATES(HOST,NAME,DATA) VALUES('sbc.megafon.ru','Multiphon',"$_[0]->{'fromdomain'}='multifon.ru'; $_[0]->{'videosupport'}='no'; $_[0]->{'fromuser'}=$_[0]->{'defaultuser'}=$_[0]->{'username'}; $_[0]->{'dtmfmode'}='inband'; $_[0]->{'disallow'}='all'; $_[0]->{'allow'}='ulaw'; $_[0]->{'port'}='5060'; $_[0]->{'insecure'}='port,invite';");
